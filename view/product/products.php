...
$cond=($_REQUEST['cid'])?" WHERE `cid`='{$_REQUEST['cid']}'":"";
$cond=($cond)?$cond." AND `lang`='$lang'":" WHERE `lang`='$lang'";
...
$sql="SELECT * FROM `product` $cond $limit;";
	...
	while ($i<$rows){
		...
		?>
			<tr class="innerCategory">
				...
				<td valign="top">
					<h3><a href="<?="../product/viewProduct.php?id=$id"?>" class="loadLink"><?=stripslashes($name)?></a></h3>
...
<div class="pagination"><?
	if(!$tp){$sql = "SELECT count(*) AS co FROM `product` $cond;";
		$result = mysql_query($sql);
