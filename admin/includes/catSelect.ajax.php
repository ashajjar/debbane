...
if($numRows>0)
{
	...
	while ($i<$numRows)
	{
		$cat_id=mysql_result($result, $i,"id");
		$cat_name=mysql_result($result, $i,"name");
		$level=getCatLevel($cat_id);
		
		?>
			<div class="<?=($level>=3)?"__leaf":"__branch"?>" id="cat_<?=$cat_id?>"><?=$cat_name?></div>
		<?
		$i++;
	}
}
...
function getCatPath($cat_id){
	if($cat_id==0){
		...
		return getCatPath(getParentCat($cat_id)).stripslashes(getDataCell("name" ,"category", "id", $cat_id))." > ";
	}
}
